...
## Обоснование выбора
### Управление доступом
Необходимо проанализировать управление доступом, которое использует компания, т.к. она оперирует чувствительными данными.
### Безопасность данных
Опять же, в связи с тем, что компания работает с конфиденциальными данными, требуется проанализировать безопасность работы с ними.
### Защита инфраструктуры
Инфраструктуру так же требуется проанализировать, т.к. очень много аспектов не освещено: устройство сети, порты, антивирусы и т.д. и т.п.
### Управление инцидентами
Компания озабочена инцидентом у конкурента и хочет избежать их повторения.
### Специфические аспекты
#### Общие вопросы
Оставляем этот блок, т.к. он регулирует взаимодействие с собственником. 
#### Управление оборудованием и физической безопасностью
Оставляем в связи с тем, что у собственника имеется собственный ЦОД и процесс взаимодействия с ним должен быть регламентирован.
...
| 1   | Используется ли многофакторная аутентификация (MFA) для доступа к критическим системам и сервисам?                                       |  Нет     |  Можно настроить в частично используемом Keycloak            |
...
| 6   | Ведётся ли журналирование всех операций доступа к системам и данным? Кто имеет доступ к журналам?                                       | Нет данных      | Может быть что-то есть в Keycloak и AD, но системного подхода нет             |
...
| 11  | Ведётся ли единый реестр учётных записей?                                                                                               | Нет       | Используется несколько систем без единного реестра             |
...
| 1   | Разработана ли политика классификации данных, и актуализируется ли она в соответствии с действующим законодательством?                  | Нет      | Требуется разработать и внедрить из-за работы с чувствительными данными              |
